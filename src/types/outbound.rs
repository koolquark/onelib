    ...
    pub fn client(&self) -> Client {
        ...
        if self.identity.is_some() {
            let identity: Identity =
                Identity::from_pem(self.identity.clone().unwrap().as_bytes()).unwrap();
        ...
        if self.auth.is_some() {
            ...
            headers.extend(Self::basic_auth_header(
                basic_auth.username,
                basic_auth.password,
            ));
        }
        ...
        match (
            ...
            (true, true, _) => match Proxy::https(self.https_proxy.as_ref().unwrap()) {
                Ok(p) => {
            ...
            (false, _, true) => match Proxy::https(self.http_proxy.as_ref().unwrap()) {
                Ok(p) => {
                    builder = builder.proxy(p);
                }
                Err(e) => {
